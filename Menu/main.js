...
btnContainer.addEventListener("click", filteredItems);
...
function filteredItems(e) {
  ...
  let filterCats = menu.filter((item) => item.category === category);
  if (category === "all") {
    displayMenu(menu);
  } else {
    displayMenu(filterCats);
  }
}

function filtercategory() {
  let cats = menu.map((item) => item.category);
  let uniqueCats = ["all", ...new Set(cats)];
  let myCats = uniqueCats
    .map((cat) => {
      return `
